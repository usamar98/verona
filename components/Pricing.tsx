...
const plans = [
  {
    ...
    yearlyPrice: 39,
    description:
      "Perfect for early-stage projects hosting occasional AMAs.",
    features: [
      "3 AMA sessions / month",
  ...
  },
  {
    ...
    yearlyPrice: 119,
    description:
      "For growing communities that need regular, professional AMAs.",
    features: [
      "Unlimited AMA sessions",
      "Up to 5,000 participants",
      "Full agent personality config",
      "RAG document upload",
  ...
  },
  {
    ...
    yearlyPrice: null,
    description:
      "Custom solutions for large protocols and enterprise-grade deployments.",
    features: [
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        <div className="text-center mb-12 space-y-5">
          ...
          <p className="text-white/50 font-outfit max-w-md mx-auto">
            No hidden fees. No on-chain surprises. Pay for what your project
            needs.
          </p>
          ...
          <div className="flex items-center justify-center gap-3 pt-2">
            <span
              className={`text-sm font-outfit ${!yearly ? "text-white" : "text-white/40"}`}
            >
              Monthly
            </span>
            <button
            ...
            <span
              className={`text-sm font-outfit flex items-center gap-2 ${yearly ? "text-white" : "text-white/40"}`}
            >
              Yearly
              <span className="bg-green-500/20 text-green-500 text-xs font-mono px-2 py-0.5 rounded-full">
                SAVE 20%
              </span>
            </span>
          </div>
              ...
              className={`relative rounded-2xl p-7 flex flex-col transition-all duration-300
                ${
                  plan.highlight
                    ...
                    : "border-white/5 hover:border-green-500/15"
                } border
              `}
              style={{
                background: plan.highlight ? "#0d1a10" : "#0d0d0d",
              }}
            >
              {/* Popular badge */}
              {plan.badge && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="bg-green-500 text-black font-mono font-bold text-xs px-3 py-1 rounded-full">
                    {plan.badge}
                  </span>
                </div>
              )}
              ...
              <div className="mb-6">
                ...
                <p className="text-xs text-white/40 font-outfit leading-relaxed">
                  {plan.description}
                </p>
              </div>
              ...
              <ul className="space-y-3 mb-8 flex-1">
                {plan.features.map((feat, j) => (
                  <li
                    key={j}
                    className="flex items-start gap-2.5 text-sm font-outfit text-white/60"
                  >
                    <svg
